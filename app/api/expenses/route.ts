...
export async function GET() {
  try {
    console.log('API: Starting expenses fetch...');
    ...
    console.log('Expenses fetched:', expenses.length, 'items');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.GOOGLE_SHEETS_SERVICE_ACCOUNT_EMAIL && process.env.GOOGLE_SHEETS_PRIVATE_KEY) {
      ...
      return NextResponse.json(
        { error: 'Service Account credentials are required for creating expenses.' },
        { status: 500 }
      );
    }
    ...
    const expense = await dataService.addExpense(body);
