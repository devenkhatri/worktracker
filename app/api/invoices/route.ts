...
export async function GET() {
  try {
    console.log('API: Starting invoices fetch...');
    ...
    console.log('Invoices fetched:', invoices.length, 'items');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.GOOGLE_SHEETS_SERVICE_ACCOUNT_EMAIL && process.env.GOOGLE_SHEETS_PRIVATE_KEY) {
      ...
      return NextResponse.json(
        { error: 'Service Account credentials are required for creating invoices.' },
        { status: 500 }
      );
    }
    ...
    const invoice = await dataService.generateInvoice(projectId, clientId);
